...
function whatsappDeliveryStatus(rule: InversionAlerta) {
  if (!rule.canal_whatsapp || rule.estado === 'normal') return null
  if (rule.ultimo_error_whatsapp) {
    return { tone: 'error' as const, text: `WhatsApp no enviado: ${rule.ultimo_error_whatsapp}` }
  }
  if (rule.ultima_entrega_whatsapp_at) {
    const deliveredAt = formatAssetDate(rule.ultima_entrega_whatsapp_at)
    return { tone: 'success' as const, text: `WhatsApp aceptado por Meta${deliveredAt ? ` · ${deliveredAt}` : ''}` }
  }
  return { tone: 'pending' as const, text: 'WhatsApp pendiente de confirmación · se reintentará cada 5 min' }
}
...
export function InvestmentNotificationAlerts({ rules, positions, portfolioReturnPct, portfolioCashEur = 0, onChanged, scenarioMode = false, onScenarioRulesChange, onScenarioPriceRefresh }: Props) {
  ...
  const portfolioAlertBase = portfolioRule?.precio_base_porcentaje ?? portfolioRule?.precio_referencia ?? null
  const portfolioAlertReturnPct = portfolioAlertBase !== null && portfolioAlertBase > 0
    ? (portfolioCurrentValue - portfolioAlertBase) / portfolioAlertBase
    : portfolioReturnPct
  const portfolioWhatsAppStatus = portfolioRule ? whatsappDeliveryStatus(portfolioRule) : null
    ...
    try {
      ...
      if (scenarioMode && onScenarioRulesChange) {
        ...
        const changes: Omit<InversionAlerta, 'id' | 'created_at'> = {
          ...
          ultima_entrega_whatsapp_at: editingRule?.ultima_entrega_whatsapp_at ?? null,
  ...
  return (
    <section className="rounded-xl bg-[#f7f5ef] p-4 text-slate-900 shadow-[0_12px_30px_rgba(0,0,0,.14)] sm:p-5" id="external-alerts">
      ...
      <div className="mt-4 grid gap-2 lg:grid-cols-2">
        <div className={`rounded-lg border p-3 ${portfolioAlertReturnPct !== null && portfolioAlertReturnPct < 0 ? 'border-rose-200 bg-[#fff1f0]' : 'border-emerald-200 bg-[#eef6e5]'}`}>
          <div className="flex items-start justify-between gap-3">
            <div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-400">Cartera completa</p><p className="mt-1 text-[13px] font-semibold">Alerta de valor total</p></div>
      ...
      {assetRules.length > 0 ? <div className="mt-4 grid items-stretch gap-3 md:grid-cols-2 xl:grid-cols-3">
        {assetRules.map((rule, index) => {
          ...
          const deliveryStatus = whatsappDeliveryStatus(rule)
          const isBusy = busyRuleId === rule.id || refreshingRuleId === rule.id
          const isRefreshing = refreshingRuleId === rule.id
          return <Slide key={rule.id} inView inViewOnce delay={Math.min(index * 55, 420)} className="h-full w-full">
            <Tilt maxTilt={4} perspective={900} className="h-full w-full">
              <Shine asChild enableOnHover enableOnTap color={displayedTarget ? '#fbbf24' : '#c8f56a'} opacity={0.22} className="h-full rounded-2xl">
                <div className="flex h-full min-w-0 flex-col gap-3 overflow-hidden rounded-2xl border border-slate-200/90 bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,0.06)] transition-shadow duration-300 hover:shadow-[0_14px_34px_rgba(15,23,42,0.12)]">
                  <div className="flex min-w-0 flex-1 flex-col"><div className="flex flex-wrap items-center gap-2"><p className="truncate text-sm font-semibold tracking-[-0.02em] text-slate-900">{name}</p><span className="rounded-md bg-slate-100 px-2 py-1 text-[9px] font-semibold text-slate-500">{ticker}</span><span className={`rounded-full px-2.5 py-1 text-[9px] font-semibold ${rule.activa ? 'bg-[#e7f2d4] text-[#31531d]' : 'bg-slate-100 text-slate-500'}`}>{position ? 'En cartera' : 'No poseído'}</span></div><div className="mt-3 grid grid-cols-2 gap-2"><div className="rounded-xl border border-slate-100 bg-slate-50/80 px-3 py-2.5"><p className="text-[9px] font-bold uppercase tracking-[0.1em] text-slate-400">Precio actual</p><div className="mt-1 flex flex-wrap items-baseline gap-x-2 gap-y-0.5"><p className="text-base font-semibold tabular-nums text-slate-800">{formatAssetPrice(presentationQuote.price, presentationQuote.currency)}</p>{referencePrice !== null && presentationQuote.currency !== 'EUR' ? <span className="text-[9px] font-medium tabular-nums text-slate-500">Referencia EUR: {formatAssetPrice(referencePrice, 'EUR')}</span> : null}</div></div><div className={`rounded-xl border px-3 py-2.5 ${currentReturnPct !== null && currentReturnPct !== undefined && currentReturnPct < 0 ? 'border-rose-100 bg-rose-50/80' : 'border-emerald-100 bg-[#eef6e5]'}`}><p className="text-[9px] font-bold uppercase tracking-[0.1em] text-slate-400">Variación actual</p><p className={`mt-1 text-base font-semibold tabular-nums ${currentReturnPct !== null && currentReturnPct !== undefined && currentReturnPct < 0 ? 'text-red-700' : 'text-emerald-700'}`}>{formatPercent(currentReturnPct)}</p><p className="mt-0.5 text-[9px] text-slate-500">{rule.precio_base_porcentaje ? `desde ${percentageBaseLabel(rule, position)} al configurar` : 'base porcentual pendiente'}</p></div></div>{displayedTarget ? <Shine asChild enableOnHover enableOnTap color="#fbbf24" opacity={0.3} className="mt-3 rounded-xl"><div className="rounded-xl border border-amber-200 bg-amber-50/80 px-3 py-2.5 shadow-sm"><div className="flex items-center justify-between gap-3"><p className="text-[9px] font-bold uppercase tracking-[0.1em] text-amber-800">Objetivo de alerta ({displayedTarget.currency})</p><span className="rounded-full bg-white/80 px-2 py-0.5 text-[9px] font-semibold text-amber-800">Nivel absoluto</span></div><p className="mt-1 text-lg font-semibold tabular-nums text-amber-950">{formatAssetPrice(displayedTarget.price, displayedTarget.currency)}</p></div></Shine> : null}<div className="mt-3 grid gap-1.5 border-t border-slate-100 pt-3"><p className="text-[10px] text-slate-500">Avisar si sube {percentInput(rule.umbral_subida_pct) || '—'}% o cae {percentInput(rule.umbral_caida_pct) || '—'}% <span className="text-slate-300">·</span> {ruleChannels(rule)}</p><p className="flex items-center gap-1.5 text-[10px] text-slate-400">{deliveryStatus ? <><span className={deliveryStatus.tone === 'error' ? 'text-red-600' : deliveryStatus.tone === 'pending' ? 'text-amber-600' : 'text-emerald-600'}>{deliveryStatus.tone === 'error' ? <CircleAlert className="h-3.5 w-3.5" /> : deliveryStatus.tone === 'pending' ? <Clock3 className="h-3.5 w-3.5" /> : <CheckCircle2 className="h-3.5 w-3.5" />}</span>{deliveryStatus.text}</> : rule.ultimo_error ? <><CircleAlert className="h-3.5 w-3.5 text-amber-600" />{rule.ultimo_error}</> : rule.ultima_comprobacion_at ? <><CheckCircle2 className="h-3.5 w-3.5 text-emerald-600" />Última comprobación {new Intl.DateTimeFormat('es-ES', { dateStyle: 'short', timeStyle: 'short' }).format(new Date(rule.ultima_comprobacion_at))}</> : 'Pendiente de la próxima comprobación'}</p></div></div>
                  <div className="mt-3 grid w-full grid-cols-2 gap-2 border-t border-slate-100 pt-3"><RippleButton type="button" size="sm" className="col-span-2 w-full border border-slate-900 bg-slate-900 text-white shadow-sm hover:bg-slate-800" onClick={() => void refreshAssetPrice(rule)} disabled={isBusy}>{isRefreshing ? <Loader2 className="animate-spin" /> : <RefreshCw />} {isRefreshing ? 'Actualizando…' : 'Actualizar precio'}<RippleButtonRipples color="#c8f56a" /></RippleButton><RippleButton type="button" size="sm" variant="outline" className="col-span-2 w-full border-slate-200 bg-slate-50 text-slate-700 shadow-sm hover:bg-slate-100" onClick={() => void updateRule(rule, { activa: !rule.activa })} disabled={isBusy}>{isBusy ? <Loader2 className="animate-spin" /> : rule.activa ? 'Pausar' : 'Activar'}<RippleButtonRipples color="#90b85f" /></RippleButton><div className="col-span-2 flex w-full gap-2 pt-1"><RippleButton type="button" size="icon" variant="outline" aria-label={`Editar alerta de ${name}`} className="h-9 w-9 rounded-lg border-slate-200 bg-white text-slate-600 shadow-sm hover:border-slate-300 hover:bg-slate-50" onClick={() => openAssetRule(rule)} disabled={isBusy}><Pencil /><RippleButtonRipples color="#90b85f" /></RippleButton><RippleButton type="button" size="icon" variant="outline" aria-label={`Eliminar alerta de ${name}`} className="h-9 w-9 rounded-lg border-rose-200 bg-rose-50 text-rose-700 shadow-sm hover:bg-rose-100" onClick={() => setRulePendingDeletion(rule)} disabled={isBusy}><Trash2 /><RippleButtonRipples color="#ef4444" /></RippleButton></div></div>
                </div>
              </Shine>
            </Tilt>
          </Slide>
