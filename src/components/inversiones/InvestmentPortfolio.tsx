  ...
  return (
    <InvestmentFrame activeTab={activeTab} onChange={cambiarPestana} statusLabel="Cartera guardada en la app">
      ...
      <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) resetOperation() }}>
        <DialogContent className="max-h-[calc(100vh-2rem)] overflow-y-auto border-slate-200 bg-[#f7f5ef] text-slate-900 sm:max-w-lg">
          ...
          <form onSubmit={guardarOperacion} className="grid gap-4">
            <div className="grid gap-3 sm:grid-cols-2"><div className="grid gap-2"><Label htmlFor="operation-type">Tipo</Label><select id="operation-type" value={operationType} onChange={(event) => handleOperationType(event.target.value as OperationType)} className="h-9 rounded-md border border-slate-200 bg-white px-3 text-sm outline-none focus:border-slate-500"><option value="Compra">Compra</option><option value="Venta">Venta</option><option value="Dividendo">Dividendo</option><option value="Aportación">Aportación</option><option value="Traspaso">Traspaso</option></select></div><div className="grid gap-2"><Label htmlFor="operation-date">Fecha</Label><Input id="operation-date" type="date" value={fecha} onChange={(event) => setFecha(event.target.value)} required /></div></div>
            <div className="grid gap-2"><Label htmlFor="operation-position">Posición existente</Label><select id="operation-position" value={selectedPosition} onChange={(event) => seleccionarPosicion(event.target.value)} className="h-9 rounded-md border border-slate-200 bg-white px-3 text-sm outline-none focus:border-slate-500"><option value="new">Nueva posición</option>{positions.map((position) => <option key={position.id} value={position.id}>{position.price_ticker || position.ticker} · {assetLabel(position)} · {position.custodia}</option>)}</select></div>
            <div className="grid gap-3 sm:grid-cols-[1.4fr_.6fr]"><div className="grid gap-2"><Label htmlFor="operation-asset">Activo</Label><Input id="operation-asset" value={activo} onChange={(event) => setActivo(event.target.value)} placeholder="Nombre del activo" required /></div><div className="grid gap-2"><Label htmlFor="operation-ticker">Ticker</Label><Input id="operation-ticker" value={ticker} onChange={(event) => setTicker(event.target.value)} placeholder="BTC, SXR8…" required /></div></div>
            <div className="grid gap-3 sm:grid-cols-2"><div className="grid gap-2"><Label htmlFor="operation-type-asset">Tipo de activo</Label><select id="operation-type-asset" value={tipoActivo} onChange={(event) => setTipoActivo(event.target.value)} className="h-9 rounded-md border border-slate-200 bg-white px-3 text-sm outline-none focus:border-slate-500"><option value="Crypto">Crypto</option><option value="Crypto / Staking">Crypto / Staking</option><option value="ETF">ETF</option><option value="Acción">Acción</option><option value="Fondo">Fondo</option><option value="Otro">Otro</option></select></div><div className="grid gap-2"><Label htmlFor="operation-custody">Custodia / broker</Label><Input id="operation-custody" value={custodia} onChange={(event) => setCustodia(event.target.value)} placeholder="Trade Republic…" required /></div></div>
            <div className="grid gap-3 sm:grid-cols-2"><div className="grid gap-2"><Label htmlFor="operation-quantity">Cantidad</Label><Input id="operation-quantity" type="number" min="0" step="any" value={cantidad} onChange={(event) => setCantidad(event.target.value)} placeholder="0,00" required /></div><div className="grid gap-2"><Label htmlFor="operation-price">Precio unitario (€)</Label><Input id="operation-price" type="number" min="0" step="any" value={precio} onChange={(event) => setPrecio(event.target.value)} placeholder="0,00" required /></div></div>
            <div className="grid gap-2"><Label htmlFor="operation-notes">Nota <span className="font-normal text-slate-400">(opcional)</span></Label><textarea id="operation-notes" value={notas} onChange={(event) => setNotas(event.target.value)} rows={3} placeholder="Comisión, motivo, referencia…" className="resize-y rounded-md border border-slate-200 bg-white px-3 py-2 text-sm outline-none placeholder:text-slate-400 focus:border-slate-500" /></div>
            <div className="flex gap-2 bg-[#eeece5] px-3 py-2.5 text-[10px] leading-relaxed text-slate-500"><Info className="mt-0.5 h-3.5 w-3.5 shrink-0 text-slate-400" />La operación queda guardada en tu cuenta. No envía órdenes al broker.</div>
